...
export const defaultPorts = () : SyncPort[] => {
  return [{ id: 'main', color: '#0f766e' }]
}

export const toNodeSizeStyle = (width: number, height: number) => {
  return {
...
export const estimateBodyHeight = (body: string, charactersPerLine = itemBodyCharactersPerLine) => {
  if (!body) {
    return 72
  }
...
const getItemWidgetHeight = (nodeId: string) => {
...
export const getMeasuredItemNodeHeight = (node: SyncNode, measuredBodyHeight?: number) => {
...
export const getMinimumNodeHeight = (node: SyncNode) => {
...
export const getMinimumNodeWidth = (node: SyncNode) => {
  if (node.type === 'section') {
...
export const withContentSizedNode = (node: SyncNode) : SyncNode => {
  if (node.type !== 'item') {
    return node
  }
...
export const normalizeNode = (node: FlowNode) : SyncNode => {
...
export const stripParentExtent = (node: SyncNode) : SyncNode => {
  const nextNode = { ...node }
...
export const createGraphCache = (nodes: SyncNode[], edges: SyncEdge[] = []) : GraphCache => {
...
export const orderNodesByHierarchy = (nodes: SyncNode[]) => {
  ...
  const sortByOriginalOrder = (candidates: SyncNode[]) => {
    candidates.sort(
      (left, right) =>
...
export const isNodeInsideSection = (
...
export const isDirectChildOfSection = (
  nodeId: string | null | undefined,
  sectionId: string,
  graph: GraphCache
) => {
...
export const shouldUseSectionThroughEdge = (
...
export const getEdgeRenderType = (
...
export const normalizeEdge = (edge: FlowEdge, graph: GraphCache) : SyncEdge => {
  return {
    id: edge.id,
    source: edge.source,
    target: edge.target,
...
export const withDefaultEdges = (flowEdges: SyncEdge[], graph: GraphCache) => {
...
export const isAncestorSection = (ancestorId: string, nodeId: string, graph: GraphCache) => {
  let current = graph.nodeById.get(nodeId)
...
export const getNodeBounds = (node: SyncNode, graph: GraphCache) => {
  const size = getNodeSize(
    node,
...
export const getRenderedNodeBounds = (node: SyncNode, graph: GraphCache) => {
...
const getBoundsArea = (bounds: { width: number, height: number }) => {
  return Math.max(1, bounds.width * bounds.height)
}

const isBoundsCoveredEnough = (
...
export const findContainingSectionForBounds = (
...
export const isNodeInsideSectionEnough = (node: SyncNode, section: SyncNode, graph: GraphCache) => {
  return isBoundsCoveredEnough(getRenderedNodeBounds(node, graph), getNodeBounds(section, graph))
}

export const setNodeParentForSection = (node: SyncNode, section: SyncNode, graph: GraphCache) => {
...
export const removeNodeFromSection = (node: SyncNode, graph: GraphCache) => {
  const absolutePosition = getAbsolutePosition(node, graph)
...
export const adoptContainedNodesIntoSection = (section: SyncNode, graph: GraphCache) => {
...
export const pruneEdgesAfterSectionExit = (
...
export const pruneInvalidSectionConnections = (edgesToFilter: SyncEdge[], graph: GraphCache) => {
  return edgesToFilter.filter((edge) => isValidSectionConnection(edge, graph))
}

export const recalculateSectionMembershipInGraph = (
  sectionId: string,
  nextNodes: SyncNode[],
...
const getMovedNodeDimensions = (node: SyncNode, dimensions?: { width: number, height: number }) => {
  if (dimensions) {
    return dimensions
  }
...
export const applySectionMembershipForMovedNodes = (
...
export const applySectionMembershipForMovedNode = (
  nodeId: string,
  absolutePosition: { x: number, y: number },
  dimensions: { width: number, height: number },
  nextNodes: SyncNode[],
...
export const isValidSectionConnection = (
  connection: {
    source?: string | null
    target?: string | null
