...
export const usePresence = (runtime: FlowRuntime, services: FlowEditorServices) => {
  ...
  const getCursorStyle = (user: SyncPresenceUser) => {
  ...
  const applyPresenceDocument = (document?: SyncPresenceDocument | null) => {
    if (!document?.users) {
      runtime.collaborators.value = []
      return
    }
  ...
  const submitPresenceUser = (user: SyncPresenceUser) => {
  ...
  const getPresenceSelectedNodeIds = () => {
  ...
  const getLocalPresenceUser = (cursor = runtime.timers.pendingCursor) : SyncPresenceUser => {
    return {
  ...
  const scheduleCursorUpdate = (position: { x: number, y: number }) => {
    if (!runtime.isLoggedIn.value) {
      return
    }
