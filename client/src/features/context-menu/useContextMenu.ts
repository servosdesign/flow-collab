...
export const useContextMenu = (runtime: FlowRuntime, services: FlowEditorServices) => {
  ...
  const openNodeMenuButton = (nodeId: string, event: MouseEvent) => {
  ...
  const openSelectionContextMenu = (payload: { event: MouseEvent, nodes: Array<{ id: string }> }) => {
    if (!runtime.isLoggedIn.value) {
      return
    }
  ...
  const openSelectedBoundsContextMenu = (event: MouseEvent) => {
    const selectedIds = getSelectedNodeIds()
