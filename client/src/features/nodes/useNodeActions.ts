...
export const useNodeActions = (runtime: FlowRuntime, services: FlowEditorServices) => {
  ...
  const getCurrentSyncEdges = (nextNodes?: SyncNode[]) => {
    return services.getCurrentSyncEdges(nextNodes)
  }

  const withSelectionState = (nodes: FlowNode[]) => {
  ...
  const refreshNodeInternalsSoon = (nodeIds: string[]) => {
    nextTick(() => {
  ...
  const setCreateMode = (kind: FlowNodeKind) => {
  ...
  const createNodeAt = (kind: FlowNodeKind, clientX: number, clientY: number) => {
  ...
  const handleCreateDragStart = (kind: FlowNodeKind, event: DragEvent) => {
    if (!runtime.isLoggedIn.value) {
      return
    }
  ...
  const handleCreateDrop = (event: DragEvent) => {
  ...
  const updateLocalNode = (nodeId: string, updater: (node: FlowNode) => void) => {
  ...
  const addNodePort = (nodeId: string) => {
    const document = runtime.flowDocument.value
  ...
  const recalculateSectionMembership = (sectionId: string) => {
    const document = runtime.flowDocument.value
  ...
  const sanitizeSectionMembership = () => {
    const document = runtime.flowDocument.value
  ...
  const getDraggedNodeMembershipChange = (draggedGraphNode: NodeDragEvent['node']) => {
  ...
  const handleNodeDragStop = (payload: NodeDragEvent) => {
    const document = runtime.flowDocument.value
  ...
  const handleSelectionDragStop = (payload: NodeDragEvent) => {
    const document = runtime.flowDocument.value
  ...
  const handleSelectionDrag = () => {
    scheduleSelectionBoundsRefresh()
  }

  const submitNodeData = (
    nodeId: string,
