...
export const useResize = (runtime: FlowRuntime, services: FlowEditorServices) => {
  const clampResizeParams = (node: SyncNode, params: ResizeParams) : ResizeParams => {
  ...
  const scheduleResizeCommit = (nodeId: string, params: ResizeParams) => {
  ...
  const flushResizeCommit = (nodeId: string, params: ResizeParams) => {
    if (runtime.timers.resizeCommitTimer) {
  ...
  const resizeNodePreview = (nodeId: string, params: ResizeParams, syncResize = true) => {
  ...
  const resizeNode = (nodeId: string, params: ResizeParams) => {
